...
| M3 | **FD-13 — run diary becomes truth (slice-3).** `workflow_run_events` through gateway + `live_outbox`; run state projected from events; chat replay moves file → DB query. Decide in-flight-run cutover here. | The store spine for FD-11; build the new engine semantics on it ONCE (don't retrofit). | FD-13, FD-11 |
...
| S4 **FD-15 concurrency setting** + **FD-16 two-tier tools** | nothing — small | FD-15, FD-16 |
