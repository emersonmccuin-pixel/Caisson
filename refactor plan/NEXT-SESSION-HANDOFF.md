...
The mailbox + worker + send-queue + routing are BUILT and the routing is **verified** (OBJ-1 below passed a live Playwright test). Two fix-pass defects are now **FIXED in code + automated gates green, PENDING live re-test**; one deep recovery is still deferred:

1. **OBJ-3 rework — echo-timeout composer-clear → FIXED (code), pending live re-test.** Root cause confirmed against the leaked CC source (`src/hooks/useTextInput.ts` `handleEscape`): the composer Esc clear is a **double-press** (`useDoublePress`, 800ms window) — a SINGLE `\x1b` only arms an "Esc again to clear" notification and leaves the body, which is exactly why the shipped single-Esc glued (`…what they asked.Any update…`). Fix = **double-Escape** with a 40ms settle gap between the two (`send-protocol.ts` echo-timeout branch). Chose double-Escape over the handoff's tentative Ctrl-U because Ctrl-U (`killToLineStart`) only clears to the current line start and would leave earlier lines of a multi-line paste; double-Escape empties the WHOLE composer (`onClearInput` + `onChange('')`) regardless of line count. Still never writes `\r`. Tests updated (`packages/runtime/test/send-protocol.test.ts`). **Live re-test still owed:** rapid sends while busy → no gluing.
2. **Drain-on-idle race → FIXED (code), pending live re-test.** Root cause: `ConversationSendService.enqueueRuntimeTurn` (the mailbox/system entry) — unlike `sendUserTurn` — never kicked a drain after enqueue, even when the PTY was `ready`. The PTY state→ready handler (`pty-handlers.ts:131`) fires the drain on the busy→ready EDGE, but a mailbox turn enqueued AFTER that edge (PTY already idle) missed it and sat queued until the next user send. (The `queued_busy` label was a red herring — `queuedStatusForState('ready', false)` returns `queued_busy` as the default queued label, NOT evidence the PTY was busy.) Fix = mirror `sendUserTurn`: `if (state === 'ready') this.deliverNextQueuedTurn(...)` in `enqueueRuntimeTurn` (`packages/app-services/src/conversations/send-service.ts`). Regression test added. **Live re-test owed:** dispatch/complete an agent while orchestrator idle → mailbox turn auto-surfaces (no manual nudge).
3. **OBJ-2 deep recovery (STILL DEFERRED, needs a live host trace).** The defensive `not-resumable → resume-failed` finalize is built (never strands `running`). The deep "keep the host run paused across its own spawn exit so the answer threads" fix needs the `PC_DEBUG_HOST_RESUME=1` `[agent-host] answer-pending … stateBefore/stateAfter` trace — capture it by pausing+answering a host-backed agent, then code the confirmed branch.

**Slice-009 live review status:** OBJ-1 ✅ verified. OBJ-3 ✅ fixed in code (was ❌ live) — **needs live re-test**. Drain race ✅ fixed in code — **needs live re-test**. OBJ-2 ⚠ defensive-only (deep deferred, needs trace). **To live-verify 1 & 2 you MUST relaunch the gated stack** — both fixes are server-side (`packages/runtime` + `packages/app-services`), so the running stack (PID-on-4040) has the OLD code; Vite-HMR does not pick them up.

**Stack note:** a gated `pnpm dev:app` (PC_DELIVERY_*=mailbox, PC_DEBUG_HOST_RESUME=1) was relaunched this session and is likely still running on 4040/5173/8788. Don't blindly relaunch; check first. Never restart unasked.
