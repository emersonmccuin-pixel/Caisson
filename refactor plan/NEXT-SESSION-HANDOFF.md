...
**EXECUTION ORDER: 010 (Areas) → 015a → 015b → 015c → 011 → 013 → 014.** Do NEXT = **slice 010 (Areas)** (`build-slices/010-areas.md`, planned, session-tracker row 36), then 015. ADR steps 1–5 are already done (agent-run `rev` + agent-host reconcile landed in 005/009), so the ADR's "agent-runs missing rev" note is stale — 015 is only steps 6–8 + the WS cut-over.
