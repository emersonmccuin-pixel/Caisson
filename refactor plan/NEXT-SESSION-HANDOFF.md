...
The mailbox + worker + send-queue + routing are BUILT and the routing is **verified** (OBJ-1 below passed a live Playwright test this session). What's left is making the **type-into-PTY delivery reliable** — two open defects + the deferred host-resume recovery:

1. **OBJ-3 rework — echo-timeout composer-clear (FAILED live).** The shipped fix writes Escape (`\x1b`) on echo-timeout (`send-protocol.ts:207`), but the live test PROVED it does NOT clear claude.exe's composer — a timed-out completion turn **glued** onto the next send (`…what they asked.Any update…`). Replace Escape with **Ctrl-U (`\x15`) + a short settle delay before the next send drains**, and read the leaked CC Ink input code to confirm the correct clear + reliable submit. Re-test live (rapid sends while busy → no gluing).
2. **Drain-on-idle race (NEW, found live).** A mailbox-injected orchestrator turn sat `queued_busy` for ~221s and never auto-drained — it only moved when the user sent another message. The worker enqueues ~350ms after the orchestrator goes idle, missing the busy→ready drain edge. Fix the drain trigger to re-check the queue after an enqueue / on the idle transition (send-queue territory — `orchestrator-send-queue-delivery.ts` + the worker). This is the bug that makes agent results silently not surface.
3. **OBJ-2 deep recovery (DEFERRED, needs a live host trace).** The defensive `not-resumable → resume-failed` finalize is built (never strands `running`). The deep "keep the host run paused across its own spawn exit so the answer threads" fix needs the `PC_DEBUG_HOST_RESUME=1` `[agent-host] answer-pending … stateBefore/stateAfter` trace — capture it by pausing+answering a host-backed agent, then code the confirmed branch.

**Slice-009 live review status:** OBJ-1 ✅ verified (agent completion → mailbox `agent-terminal`, delivery `accepted` orchestrator-turn via send-queue, no `agent_inbox` row). OBJ-3 ❌ failed (above). OBJ-2 ⚠ defensive-only (deep deferred). So slice 009 is NOT cleanly done — the above is its fix pass.
