...
- **Track A (runtime):** Theme 1 → Theme 2 — `apps/server` host client + reconcile.
- **Track B (live-UI/delivery):** Theme 3 — `apps/web` hooks + chat-reducer + delivery-routing.
- **Track C (contracts):** Slice 013 → Slice 014 (014 also waits on Track A's T1 for live-verify).

**Hard constraints if parallelizing:** (1) separate git **worktree/branch per track** — parallel sessions on one branch is exactly how stray edits land in a dirty tree (happened 2026-06-01 with pod-content files); (2) **only ONE session live-verifies at a time** — one dev stack + one Playwright browser, concurrent live checks collide; (3) **token budget** — ~2 tracks is the practical ceiling. **Recommended:** run 61 → Theme 1 solo, then optionally split Track B ∥ Track C.

## Stale tracker rows (NOT real work — ignore)
- Rows 17/19 (plan/review 004), 22/25/28/31 (verify/close 005–008), 35 (plan 010): the slices shipped + were reviewed live; only the formal checkboxes were never ticked. Leftover defects → folded into the foundational backlog / 013 / 014.
- Rows 41/42/43 (slice 012): **DEAD** — subsumed by 015c ("do not run 012 separately").

## Carry-overs worth a glance (all folded into the plan above)
- The `writer` one-word task idled `running` ~90s, never self-completed (Theme 1 / T1-B; known host-dispatch idle).
- Three pod-content source files (`orchestrator-pod-content.ts`, `stock-pod-seed.ts`, `workflow-builder-pod-content.ts`) were left dirty by a parallel actor (caisson pod gaining workflow-authoring) — uncommitted, untouched by the 018 work. Decide keep/discard (Theme 4 / T4-A).
