...
| Work items stages fields events build-slice plan | planned | `refactor plan/build-slices/003-work-items-stages-fields-events.md` | Work items/stages/fields/attachments/server/web/MCP/live events | Slice 002, work-items handoff, definitive session pathway | Created in manual Session 14. Scope is work-item/stage/field/attachment shared contracts, compatibility row<->DTO adapters, a server-owned mutation gateway as the single durable write door (routing the bypassing verification/auto-advance/DAG/legacy fields-only paths through it), and durable `work-item.changed` / `stage.list.changed` live events on the slice-002 `live_outbox` with `field-schema.list.changed` / `attachment.changed` payload contracts plus legacy websocket projections. Reuses the slice-002 outbox/replay route/web live client; adds no new DB migration. Next session is Session 15: build slice 003 only. |
