...
> Status: **IMPLEMENTED + LIVE-VERIFIED 2026-06-01** (commit 3753c876; Claude drove the review via Playwright + API at user request, gated stack reloaded for the new source). Gates green. Verified live: Focus tab (renamed from Dashboard) + create-Area UI write path; filter rail (All/each Area/Uncaptured + live counts) in Kanban; inspector Area dropdown assign→Save→counts update; rail filter narrows the board to the assigned item; **two-tab live propagation** (create in tab A → tab B updated with NO reload, via the relay); `area.changed` confirmed on the durable outbox door; **delete-Area falls members back to Uncaptured** (proven via both API + UI data); 0 console errors. Deferred (non-blocking): "Update with agent" summary button (optional fast-follow); web area filter is client-side (backend `?areaId=` exists, unused); delete-Area emits only `area.changed(deleted)` → UI refetches the work-item list on that reason; Focus "Uncaptured" count uses the full WI list vs the rail's visible-list count (cosmetic).
