...
> - MOVED TO 019 — server wiring (`agent-verification.ts`: wider context, `hasGitDiff`/tool-call executors, fail-closed branch). Rationale: it's inert until dispatch authors **v2** contracts (019), and landing it alone ships unverifiable plumbing. Folded into 019 so the whole path (author → run → read evidence → fail-closed) is live-verified as one unit. 014a = the domain engine only, which is done + unit-proven.
