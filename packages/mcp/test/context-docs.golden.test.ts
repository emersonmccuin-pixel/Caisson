...
test('pc_get_context_doc default: uses current project path', async () => {
  ...
  assert.equal(calls[0].path, '/api/projects/P01/context-docs/DOC1');
});
...
test('pc_get_context_doc with targetProjectId: uses target project path', async () => {
  ...
  assert.equal(firstText(res), serverBody);
  assert.equal(calls[0].path, '/api/projects/P02/context-docs/DOC1');
});
...
test('pc_get_context_doc with targetProjectId slug: passes slug in path', async () => {
  ...
  assert.equal(calls[0].path, '/api/projects/other-project/context-docs/DOC2');
});
