...
| ~~`agent-inbox.ts`~~ | ✅ deleted in M4a with its tables |
...
- `work_items.body` — **KEEP — and since M5 (2026-06-04) it is the BRIEF ONLY** (FD-5 law): ☠ `store: work_item_body` — deliverables never land in the body; `$root.output` still reads it and now reliably means "the card's brief". Round-trip guard: `m5-root-output-round-trip.test.ts`.
...
- ~~**`agent_inbox` is alive in the DB, dead in TypeScript, live in a hook.**~~ ✅ M4a (2026-06-04):
  the "alive in a hook" half was an illusion — the hook only READ rows nothing had written since
  slice 017. Hook + repo + tables deleted (0041 archive); the mailbox is the one delivery system.
- ~~**`work_items.body` does double duty.**~~ ✅ M5 (2026-06-04): refute showed the "double duty" was nearly theoretical — 0 live workflow defs used `store: work_item_body` (2 contracts ever). The store variant is DELETED; body = the human brief only; `$root.output` reads it as exactly that. Guard: `m5-root-output-round-trip.test.ts` + banned `work_item_body`.
...
- ~~Mailbox cutover gate~~ ✅ closed in M4a (2026-06-04): hook + `agent_inbox` deleted; the mailbox is the one delivery system.
