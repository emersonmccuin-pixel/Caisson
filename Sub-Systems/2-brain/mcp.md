...
| **agent-run** | Dispatch agents, continue them, pause-and-ask, inspect/kill runs, read your contract (M5), submit finished work (☠ FD-6/M7 `pc_ask_user` — ONE ask door) | `pc_invoke_agent`, `pc_continue_agent`, `pc_list_my_runs`, `pc_inspect_agent_run`, `pc_kill_agent_run`, `pc_ask_orchestrator`, `pc_request_approval`, `pc_answer_pending`, `pc_get_contract`, `pc_submit_deliverable` |
